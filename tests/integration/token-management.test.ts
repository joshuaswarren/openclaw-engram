...
const TOKENS_SRC = path.join(ROOT, "packages/engram-core/src/tokens.ts");

// Use a temp dir so we don't pollute the real tokens.json
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "engram-token-test-"));
const tmpTokensPath = path.join(tmpDir, "tokens.json");

// We can't import @engram/core directly from node:test runner without tsx,
...
test("tokens.ts uses engram_ prefix convention for known connectors", () => {
  const content = fs.readFileSync(TOKENS_SRC, "utf-8");
...
test("tokens.ts default path is ~/.engram/tokens.json", () => {
  const content = fs.readFileSync(TOKENS_SRC, "utf-8");
  assert.ok(content.includes('.engram", "tokens.json"'), "Default path must be ~/.engram/tokens.json");
});
