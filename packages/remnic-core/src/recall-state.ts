...
export interface LastRecallSnapshot {
  ...
  recordedAt: string;
  /**
   * Collision-safe write nonce (issue #518 slice 3c / Codex P2 on
   * #540).  Generated by `record()` via `randomUUID()` so every write
   * has a unique identifier even when `profilingEnabled: false`
   * (empty traceId) and `recordedAt` collides at millisecond
   * precision.  Used by `annotateTierExplain` as the authoritative
   * stale-snapshot guard; older snapshots persisted before this
   * field fall back to the traceId / recordedAt chain.
   */
  writeNonce?: string;
  queryHash: string;
  queryLen: number;
...
export class LastRecallStore {
  ...
  async record(opts: {
    ...
    const liveSnapshot: LastRecallSnapshot = {
      ...
      recordedAt: now,
      // Collision-safe write nonce — see `LastRecallSnapshot.writeNonce`.
  ...
  async annotateTierExplain(
    ...
    if (!current) return;
    if (expected) {
      // Codex P2 (round 2) on #540: the recordedAt fallback is only
      // millisecond-precision, so two back-to-back recalls on the same
      // sessionKey can share a timestamp and let a stale observation
      // pass the guard.  The `writeNonce` field (randomUUID at write)
      // is the authoritative collision-safe guard — prefer it when
      // both the expected identity and the stored snapshot carry one.
      if (
        typeof expected.writeNonce === "string" &&
        expected.writeNonce.length > 0
      ) {
        // When the caller captured a nonce, the stored snapshot must
        // also carry one and they must match exactly.  A missing nonce
        // on the stored snapshot means the snapshot was overwritten by
        // an older code path that didn't populate it — reject rather
        // than silently falling through to the weaker guard.
        if (current.writeNonce !== expected.writeNonce) return;
      } else {
        // Legacy / backward-compat path: caller didn't capture a nonce
        // (e.g. an older observation enqueue site, or the snapshot was
        // created before this field existed).  Fall back to the
        // traceId → recordedAt chain.
        //
        // Codex P1 on #540: treat empty `traceId` as missing.  When
        // `profilingEnabled: false` (the default), `startTrace()`
        // returns `""` and every snapshot persists the same empty
        // trace id, so a simple `expected.traceId !== undefined` check
        // would always be truthy and admit stale observations.
