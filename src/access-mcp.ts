...
export class EngramMcpServer {
  ...
  /** Session ID generated by the most recent initialize handshake, read by the HTTP handler. */
  lastInitSessionId?: string;
  ...
  async handleRequest(request: JsonRpcRequest, options?: { principalOverride?: string; sessionId?: string }): Promise<Record<string, unknown> | null> {
    ...
    if (method === "initialize") {
      ...
      // Store session ID for the HTTP handler to read, avoiding metadata
      // leaking into the response object (which stdio transport writes directly).
      this.lastInitSessionId = newSessionId;
      return {
        jsonrpc: "2.0",
        id,
        result: {
