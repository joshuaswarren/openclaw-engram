...
### Fixed
- Frontmatter link serialization now preserves parser symmetry for `links[].reason`: reasons are JSON-escaped when written and JSON-unescaped when parsed, preventing backslash amplification across save/load cycles while still addressing incomplete escaping.
