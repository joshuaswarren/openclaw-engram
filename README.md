...
- **Temporal recall (`valid_at` / `invalid_at`)** — Every fact carries explicit `valid_at` and `invalid_at` YAML frontmatter fields; `remnic recall "<q>" --as-of <timestamp>` returns the fact valid at that time, ignoring later supersessions. Supersession writes flip `invalid_at` automatically. See [Temporal Recall](docs/temporal-recall.md). (issue #680)
- **Free-form tag filter on recall** — `remnic remember "<text>" --tag wedding --tag q2-planning` and `remnic recall "<q>" --tag q2-planning [--tag-match all|any]`. Tags are flat strings additive to the MECE taxonomy. Tags surface in Recall X-ray output. See [Tags](docs/tags.md). (issue #689)
