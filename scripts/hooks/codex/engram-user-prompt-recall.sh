#!/usr/bin/env bash
# Codex UserPromptSubmit hook: recall Engram context for each user message.
...
LOG="${HOME}/.codex/logs/engram-user-prompt-recall.log"
mkdir -p "$(dirname "$LOG")"
...
if [ $CURL_EXIT -ne 0 ] || [ -z "$RESPONSE" ]; then
  log "recall failed (curl exit $CURL_EXIT) — passing through"
  echo '{"continue":true}'
  exit 0
fi
